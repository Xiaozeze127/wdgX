...
exports.wdgxg = function (options) {
    ...
    async function gci(dir, callback, finish) {
        fs.readdir(dir, function (err, files) {
            ...
            if (packageObj.cttp[0]["YN"]) {
                cl += '    var title = document.getElementById("' + packageObj.cttp[0]["id"] + '");\n'
                cl += '    title.innerHTML += \' - \' + tp\n'
            }
            if (packageObj.hcttp[0]["YN"]) {
                cl += '    var htitle = document.getElementById("' + packageObj.hcttp[0]["id"] + '");\n'
